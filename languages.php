<?php

return [
    "EN" => [
        "API_ERROR_WHILE_FETCHING_ZONES" => "ERROR - API error while fetching zones from cpanel",
        "API_ERROR_WHILE_UPDATING_IP" => "ERROR - unknown error while updating IP",
        "CACHE_EQUAL_REMOTE_MESSAGE" => "OK    - My IP is equal than local cache",
        "DNS_IP_UPDATED_MESSAGE" => "OK    - IP updated to: ",
        "REAL_EQUAL_DOMAIN_MESSAGE" => "OK    - My IP is equal than cPanel DNS",
        "RETRIVE_ERROR_MESSAGE" => "ERROR - Cannot read my IP remotely",
        "ZONE_NOT_FOUND" => "ERROR - Zone not found",
        "NO_IP_TYPE_SUBDOMAINS" => "ERROR - No subdomains are configured for the ip type"
    ],

    "PT_BR" => [
        "API_ERROR_WHILE_FETCHING_ZONES" => "ERROR - Erro de api ao buscar zonas DNS",
        "API_ERROR_WHILE_UPDATING_IP" => "ERROR - Erro desconhecido de api ao atualizar IP",
        "CACHE_EQUAL_REMOTE_MESSAGE" => "OK   - Meu ip é igual ao cache local",
        "REAL_EQUAL_DOMAIN_MESSAGE" => "OK   - Meu IP é igual ao DNS no cPanel",
        "RETRIVE_ERROR_MESSAGE" => "ERRO - Não foi possível ler meu ip remotamente",
        "DNS_IP_UPDATED_MESSAGE" => "OK   - IP atualizado para: ",
        "ZONE_NOT_FOUND" => "ERRO - Zona não encontrada",
        "NO_IP_TYPE_SUBDOMAINS" => "ERROR - Nenhum subdomínio foi configurado para o tipo de IP"
    ]
];
